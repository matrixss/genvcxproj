...
def generate_guid(input_str):
    ...
    guid = '{{{:0>8}-{:0>4}-{:0>4}-{:0>4}-{:0>12}}}'.format(hash_value[:8], hash_value[8:12], hash_value[12:16], hash_value[16:20], hash_value[20:32])
    return guid
...
def print_project_property_group(file):
    ...
    for platform in support_platforms:
        for configuration in support_configurations:
            file.write(f"  <PropertyGroup Condition=\"'$(Configuration)|$(Platform)'=='{configuration}|{platform}'\" Label=\"Configuration\">\n")
...
def print_project_build_tools(file):
    ...
    for platform in support_platforms:
        for configuration in support_configurations:
            file.write(f"  <PropertyGroup Condition=\"'$(Configuration)|$(Platform)'=='{configuration}|{platform}'\">\n")
...
def printheader(file, proj_guid):
    ...
    file.write("<Project DefaultTargets=\"Build\" ToolsVersion=\"14.0\" xmlns=\"http://schemas.microsoft.com/developer/msbuild/2003\">\n")
...
# </ClInclude>
def process_files_for_types(doc, dir_path, rel_path, processed_dirs, filter_group_dirs, subfolder_item_group, item_groups, file_types_map):
    global filter_other_list
    for filter_name, details in file_types_map.items():
        if filter_name == "Resource Files": #将Makefile放在资源文件分组中。
            files = [f for f in os.listdir(dir_path) if any(f.endswith(ext) for ext in details['extensions']) or f in filter_other_list]
        else:
        ...
        if files:
            if len(rel_path) > 0:
                ...
                for part in rel_path_parts:
                    ...
                    if filter_part_rel_path not in filter_group_dirs:
                        create_filter_element(doc, subfolder_item_group, filter_part_rel_path, generate_guid(filter_part_rel_path))
            ...
            for file in files:
                ...
                create_file_element(doc, item_groups[details['group']], details['element'], file_rel_path, filter_rel_path)
...
def generate_vcxproj_filters(input_dir, output_file):
    ...
    # 添加3个默认的分组。为根目录下的文件
    create_filter_element(doc, subfolder_item_group, "Source Files", "{4FC737F1-C7A5-4376-A066-2A32D752A2FF}", "cpp;c;cc;cxx;def;odl;idl;hpj;bat;asm;asmx")
    create_filter_element(doc, subfolder_item_group, "Header Files", "{93995380-89BD-4b04-88EB-625FBE52EBFB}", "h;hpp;hxx;hm;inl;inc;xsd")
    create_filter_element(doc, subfolder_item_group, "Resource Files", "{67DA6AB6-F800-4c08-8B7A-83BB121AAD01}", "rc;ico;cur;bmp;dlg;rc2;rct;bin;rgs;gif;jpg;jpeg;jpe;resx;tiff;tif;png;wav;mfcribbon-ms")
    ...
    for root, dirs, files in os.walk(input_dir, topdown=True):
        if root == input_dir:
            process_files_for_types(doc, root, "", processed_dirs, filter_group_dirs, subfolder_item_group, item_groups, group_file_types_map)
        ...
        for name in dirs:
            ...
            process_files_for_types(doc, dir_path, rel_path, processed_dirs, filter_group_dirs, subfolder_item_group, item_groups, group_file_types_map)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--copy_local_files_to_remote", help="copy local files to remote")
    parser.add_argument("-f", "--config_file", help="specify the name of an optional configuration file, if not specified, use config.json", default="config.json")
    parser.add_argument("-i", "--include_search_path", help="specify the path to search for include files not in the project directory", default="")
